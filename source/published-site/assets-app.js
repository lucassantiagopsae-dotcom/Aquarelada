(function () {
  ...
  function card(play) {
    ...
    const itemHref = `item.html?slug=${play.slug}${sourceTag ? `&tag=${encodeURIComponent(sourceTag)}` : ""}`;
    return `
  ...
  function initCapture() {
    ...
    form.addEventListener("submit", async function (event) {
      ...
      try {
        ...
        window.location.href = manualUrl("acesso.html");
      }
    });
  }

  function manualUrl(page) {
    const cleanPage = String(page || "").replace(/\.html$/, "");
    return window.location.pathname.startsWith("/supermanual")
      ? `/supermanual/${cleanPage}`
      : page;
  }
  ...
  function initAccessSearch() {
    ...
    form.addEventListener("submit", function (event) {
      ...
      window.location.href = value ? `brincadeiras.html?q=${encodeURIComponent(value)}` : "brincadeiras.html";
    });
  }
  ...
  function initBrincadeiras() {
      ...
      app.innerHTML = `
        <div class="subcategory-grid">
          ${playFilters.map((filter) => `
            <a class="category-card subcategory-card" href="brincadeiras.html?tag=${encodeURIComponent(filter.tag)}" style="--accent:${accentFor(filter.tag)}">
        ...
        <div class="actions-row page-end-actions"><a class="btn light" href="acesso.html">Voltar</a></div>
      `;
      return;
    }
    ...
    app.innerHTML = `
      ...
      <div class="actions-row page-end-actions">
        <a class="btn light" href="brincadeiras.html">Voltar às subcategorias</a>
        <a class="btn light" href="acesso.html">Voltar</a>
      </div>
    `;
  ...
  function initItem() {
    ...
    const returnHref = returnTag ? `brincadeiras.html?tag=${encodeURIComponent(returnTag)}` : "brincadeiras.html";
    app.innerHTML = `
  ...
  function initGrouped() {
      ...
      mount.innerHTML = `
        <div class="subcategory-grid">
          ${content.blocks.map((block, index) => `
            <a class="category-card subcategory-card" href="${location.pathname.split("/").pop()}?grupo=${index}" style="--accent:${["#f4bd45", "#2b8fb8", "#4f9f68", "#7b6cc7", "#ef6b5a", "#2b8fb8"][index % 6]}">
    ...
    mount.innerHTML = `
      ...
      <div class="actions-row page-end-actions">
        <a class="btn light" href="${location.pathname.split("/").pop()}">Voltar às subcategorias</a>
        <a class="btn light" href="acesso.html">Voltar</a>
      </div>
    `;
  }
